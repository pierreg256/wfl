...
(function() {
  var ActivityResponse, Application, DecisionResponse, Swf, amazon, awssum, createApplication, inspect, path, wfl_history, _checkActivity, _checkDomain, _checkWorkflow, _getEventFromId, _getProp, _makeRoute;
  ...
  Application = (function() {
    ...
    function Application(options) {
      var swfCfg, winston, _base, _base1, _base2, _base3, _base4, _base5, _base6, _base7, _base8, _ref, _ref1, _ref10, _ref11, _ref12, _ref13, _ref14, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9,
        _this = this;
      ...
      this.config.save(function(err) {
        if (err != null) {
          return inspect(err, "Config save error");
        }
      });
      if (options == null) {
        options = {};
      ...
      if ((_ref2 = (_base1 = this.options).accessKeyId) == null) {
        _base1.accessKeyId = (_ref3 = this.config.get("accessKeyId")) != null ? _ref3 : "BAD_KEY";
      }
      if ((_ref4 = (_base2 = this.options).secretAccessKey) == null) {
        _base2.secretAccessKey = (_ref5 = this.config.get("secretAccessKey")) != null ? _ref5 : "BAD_SECRET_KEY";
      }
      if ((_ref6 = (_base3 = this.options).region) == null) {
        _base3.region = (_ref7 = this.config.get("region")) != null ? _ref7 : "us-east-1";
      }
      if ((_ref8 = (_base4 = this.options).domain) == null) {
        _base4.domain = (_ref9 = this.config.get("domain")) != null ? _ref9 : "sample-domain";
      }
      if ((_ref10 = (_base5 = this.options).name) == null) {
        _base5.name = (_ref11 = this.config.get("name")) != null ? _ref11 : "sample-workflow";
      }
      if ((_ref12 = (_base6 = this.options).decider) == null) {
        _base6.decider = {};
      }
      if ((_ref13 = (_base7 = this.options.decider).name) == null) {
        _base7.name = "" + this.options.domain + "-" + this.options.name + "-decider";
      }
      if ((_ref14 = (_base8 = this.options.decider).taskList) == null) {
    ...
    Application.prototype._listenForActivity = function(name, taskList) {
      ...
      return this.swf.PollForActivityTask(swfCfg, function(err, data) {
        var body, i, request, response, token, _ref, _ref1;
        if (err != null) {
          _this.logger.error("Unexpected Error polling " + swfCfg.TaskList.name + " ", err);
        } else {
          body = data.Body;
          ...
          if (!(token != null)) {
            ...
            response = new ActivityResponse(_this, token);
            for (i in _this.options.activities) {
              if (_this.options.activities[i].name === request.name) {
                _this.logger.debug("Running the following activity: " + _this.options.activities[i].name + " ");
                _this.options.activities[i].activityTask(request, response);
              }
            }
          }
        }
          ...
          if (token != null) {
            ...
            _makeRoute(body.events, function(routeError, request) {
              ...
              for (tmpRoute in _this.options.decider.routes) {
                _results.push(_this.options.decider.routes[tmpRoute].route === request.url ? (_this.logger.debug("Making following decision: " + _this.options.decider.routes[tmpRoute].route), _this.options.decider.routes[tmpRoute].decisionTask(request, response)) : void 0);
              }
              return _results;
            });
          }
  ...
  _makeRoute = function(events, callBack) {
    var evt, evt_tool, fini, handled, history, i, info, lastActivityId, pos, request, response, route, source_evt, task, tmp, _i, _len, _ref, _ref1, _ref2, _ref3, _ref4;
    if (events == null) {
      events = [];
    }
    request = {};
